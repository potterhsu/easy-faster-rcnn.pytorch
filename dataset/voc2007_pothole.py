...
class VOC2007(Base):
    ...
    def __init__(self, path_to_data_dir: str, mode: Base.Mode, image_min_side: float, image_max_side: float):
        ...
        if self._mode == VOC2007.Mode.TRAIN:
            path_to_image_ids_txt = os.path.join(path_to_imagesets_main_dir, 'train.txt')
    ...
    def __getitem__(self, index: int):# -> Tuple[str, Tensor, Tensor, Tensor, Tensor]:
        ...
        bboxes = [obj.bbox.tolist() for obj in annotation.objects if not obj.difficult and obj.name in VOC2007.CATEGORY_TO_LABEL_DICT.keys()]
        labels = [VOC2007.CATEGORY_TO_LABEL_DICT[obj.name] for obj in annotation.objects if not obj.difficult and obj.name in VOC2007.CATEGORY_TO_LABEL_DICT.keys()]
        ...
        for c in range(1, VOC2007.num_classes()):
            ...
            try:
                ...
                _, _, ap = voc_eval(detpath=os.path.join(path_to_results_dir, 'comp3_det_test_{:s}.txt'.format(category)),
                                    ...
                                    imagesetfile=os.path.join(path_to_main_dir, 'trainval.txt'),
                                    classname=category,
